...
//数値を文字列に変換 マイバック戦略対応
void s_i(int n, char *buf){ sprintf(buf,"%d",n);}

//数値を文字列に変換 整数の最大値に対応　要：後始末
char *sI(int n){char buf[11]; sprintf(buf,"%d",n); return fh_line(buf);}
char *sN(int n){char buf[11]; sprintf(buf,"%d",n); return fh_line(buf);}
...
char *join(char **ss, int sz, int ws){
  char line[ws*sz], word[ws];
...
void joinT(){ char *arg[] = {"1","2","3"}; char *act = join(arg, 3, WORD); ez_t("join", act, "1\n2\n3","I hope so..");}
